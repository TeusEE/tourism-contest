...
describe("analysis input validation", () => {
  ...
  it("calculates quick dates in the Korea time zone", () => {
    const now = new Date("2026-12-31T15:30:00Z");
    expect(localDateString(0, now)).toBe("2027-01-01");
    expect(localDateString(1, now)).toBe("2027-01-02");
  });
});
