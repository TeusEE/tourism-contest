...
사용자가 `서울역`, `부산역`처럼 알고 있는 장소명이나 주소를 입력할 때 입력창에서 후보를 표시한다. 백엔드는 NAVER API HUB Local Search 결과의 장소명, 지번 주소, 도로명 주소와 WGS84 좌표를 반환한다. 사용자가 후보를 선택하면 입력값을 도로명 주소로 바꾸어 분석 요청에 사용한다. 웹·모바일은 300ms debounce를 적용하고, 사용자가 Enter/Return을 누르면 대기 중인 후보 검색을 즉시 실행한다. 숫자가 포함된 주소도 후보 검색을 시도하며, 후보가 없을 때는 주소 직접 입력으로 계속할 수 있다.
