...
출발지·여행지 입력은 `서울역`, `부산역` 같은 장소명을 300ms debounce로 서버에 조회한다. 후보가 여러 개인 경우 자동으로 임의 선택하지 않고 사용자가 장소·도로명 주소를 선택한다. 숫자가 포함된 완성형 도로명 주소는 추천 호출을 생략한다. 후보 API 장애 시 주소 직접 입력은 계속 사용할 수 있다.
