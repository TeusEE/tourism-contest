...
후보가 여러 개인 장소명이나 주소를 자동으로 확정하지 않는다. 사용자가 Enter로 후보를 확인한 뒤 정확한 장소·도로명 주소를 선택하도록 하며, 후보 검색이 실패해도 도로명 주소를 직접 입력해 분석을 계속할 수 있다.
...
- 입력값 trim 후 2~80자이면 장소명과 숫자가 포함된 도로명 주소 모두 추천을 시도한다.
- 입력이 멈춘 뒤 300ms debounce를 적용하고, 사용자가 Enter를 누르면 대기 중인 후보 검색을 즉시 실행한다.
...
- 후보가 표시되면 먼저 정확한 후보를 선택하도록 하고, 후보가 없거나 추천에 실패한 경우에는 후보 선택 없이도 입력값이 주소라면 분석 제출을 허용한다.
